...
// allocate aligned 2D vectors
    ...
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM17_data(HBM_link_size, 0);
...
// store data
    ...
    for (int i = 0; i < (int) cur_element_count; i++) {
        ...
        for (int j = 0; j < 16; j++) {
            ...
            HBM16_data[i * 16 + j] = gnd_link_0;
            HBM17_data[i * 16 + j] = gnd_link_1;
        }
    }
		...
		if (element_levels_[i] != 0) {
            for (int k = 0; k < (int) (element_levels_[i]); k++) {
                ...
                for (int m = 0; m < (int) maxM_; m++) {
                    ...
                    DDR0_data[index_base + k * maxM_ + m] = high_link;
                    // if (i==enterpoint_node_) {
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    OCL_CHECK(err, err = krnl_vector_add.setArg(8, buffer_HBM16_data));
    OCL_CHECK(err, err = krnl_vector_add.setArg(9, buffer_HBM17_data));
    ...
    OCL_CHECK(err, err = krnl_vector_add.setArg(10, buffer_DDR0_data));
    OCL_CHECK(err, err = krnl_vector_add.setArg(11, buffer_DDR1_data));

	OCL_CHECK(err, err = krnl_vector_add.setArg(12, enterpoint_node_));
	OCL_CHECK(err, err = krnl_vector_add.setArg(13, (int)cur_element_count));
	OCL_CHECK(err, err = krnl_vector_add.setArg(14, maxlevel_));
    OCL_CHECK(err, err = krnl_vector_add.setArg(15, ef_));
    OCL_CHECK(err, err = krnl_vector_add.setArg(16, index_link));
    OCL_CHECK(err, err = krnl_vector_add.setArg(17, q_size));
//////////////////////////////   TEMPLATE END  //////////////////////////////
// ------------------------------------------------------
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    OCL_CHECK(
        ...
        // buffer_HBM8_data, buffer_HBM9_data, buffer_HBM10_data, buffer_HBM11_data, buffer_HBM12_data, buffer_HBM13_data, buffer_HBM14_data, buffer_HBM15_data, 
        buffer_HBM16_data, 
        buffer_HBM17_data, 
...
// --------------------------------------------------
    ...
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_DDR1_data},CL_MIGRATE_MEM_OBJECT_HOST));
    OCL_CHECK(err, err = q.finish());
...
// #endif
    ...
    for (int i = 0; i < q_size; i++) {
        int externalLabel = 0;
        int internal_id = DDR1_data[ddr_count+2];
